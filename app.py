...
import pickle
from streamlt_opinion_menu import option_menu
...
models = {
    ...
    'lung_cancer': pickle.load(open('Models/lung_disease_model.sav', 'rb')),
...
def display_input(label, tooltip, key, type="text"):
    if type == "text":
        return st.text_input(label, ley=key, help=tooltip)
    elif type == "number":
...
if selected == 'Diabetes Prediction':
    st.title('Diabetes')
    st.write("Enter the following details tp predict diabetes:")

    pregnancies = display_input('Number of pregnancies', 'Enter number of times of pregnant', 'Pregnancies', 'number')
    Glucose = display_input('Glucose Level', 'Enter glucose level', 'Glucose', 'number')
    BloodPressure = display_input('Blood Pressure value', 'Enter blood pressure value', 'number')
...
if st.button('Diabetes Test Result'):
    diab_prediction = models['Diabetes'].predict([[pregnencies, Glucose, BloodPressure, SkinThickness, Insulin, BMI, DiabetesPedigreeFunction, Age]])
    diab_diagnosis = 'The person id diabetic'
...
if selected == 'Heart Disease Prediction':
    ...
    cp = display_input('Chest Pain types (0, 1, 2, 3)', 'Enter chest pain type', 'cp', 'nummber')
    ...
    fbs = display_input('Fasting Blood Sugar', 'fbs', 'number')
    restecg = display_input('Resting Electrocardiographic results (0, 1, 2)', 'Enter resting ECG results', 'restecg', 'number')
    thalach = display_input('Maximum Heart Rate acheived', 'Enter maximum heart rate', 'thalach', 'number')
    exang = display_input('Exercise Induced Angina (1 = yes; 0 = no)', 'Enter maximum heart rate', 'thalach', 'number')
    ...
    ca = display_input('Major vessels colored by fluroscopy (0-3)', 'Enter number of major vessels', 'ca', 'number')
    thal = display_input('Thal (0 = normal; 1 = fixed; 2 = reversible defect)', 'Enter thal value', 'thal', 'number') 

heart_diagnosis = ''
if st.button('Heart Disease Test Result'):
    heart_prediction = models['heart_disease'].predict([[age, sex, cp, testbps, chol, fbs, restecg, thalach, exang, oldpeak, slope, ca, thal]])
...
if selected == "Parkinsons Prediction":
    ...
    PPQ = display_input('Jitter:PPQ', 'Enter Jitter:PPQ value', 'PPQ', 'number')
    ...
    if st.button("Parkinson's Test Result"):
        parkinsons_prediction = models['parkinsons'].predict([[fo, fhi, Jitter_percent, Jitter_Abs, RAP, DDP, Shimmer, Shimmer_dB, APQ3, APQ5, APQ, DDA, NHR, HNR, RPDE, DFA, spread1, spread2, D2, PPE]])
...
if selected == "Lung Cancer Prediction":
    ...
    PEER_PRESSURE = display_input('Peer Pressure (1 = Yes; 0 = No)', 'Enter if the person os under perr pressure', 'PEER_PRESSURE', 'number')
    ...
    SWALLOING_DIFFICULTY = display_input('Swalloing of Difficulty (1 = Yes; 0 = No)', 'Enter if the person has difficulty swalloing', 'SWALLOWING_DIFFICULTY', 'number')
    CHEST_PAIN = display_input('Chest Pain (1 = Yes; 0 = No)', 'Enter if the person experiences chest pain', 'CHEST_PAIN', 'number')

    lungs_diagnosis = ''
    if st.button("Lung Cancer Test Result"):
        lungs_prediction = models['lung_cancer'].predict([[GENDER, AGE, SMOKING, YELLOW_FINGERS, ANXIETY, PEER_PRESSURE, CHRONIC_DISEASE, FATIGUE, ALLERGY, WHEEZING, ALCOHOL_CONSUMING, COUGHING, SHORTNESS_OF_BREATH, CHEST_PAIN]])
        lungs_diagnosis = "The person hass lung cancer disease" if lungs_prediction[0] == 1 else "The person does not have lung cancer disease"
...
if selected == "Hypo-Thyroid Prediction":
    ...
    age = display_input('Age', 'Enter age of the person', 'age', 'number')
    sex = display_input('Sex (1 = Yes; 0 = No)', 'Enter sex of the person', 'sex', 'number')
    on_tyroxine = display_input('On Thyroxine (1 = Yes; 0 N0)', 'Enter if the person is on thyroxine', 'on_thyroxine', 'number')
    tsh = display_input('TSH Level', 'Enter TSH level', 'tsh', 'number')
    t3_measured = display_input('T3 Measured', 't3_measured', 'number')
    t3 = display_input('T3 Level', 'Enter T3 level', 't3', 'number')
    tt4 = display_input('TT4 Level', 'Enter TT4 level', 'tt4', 'number')

    thyroid_diagnosis = ''
    if st.button("Thyroid Test Result"):
        thyroid_prediction = models['thyroid'].predict([[age, sex, on_tyroxine, tsh, t3_measured, t3, tt4]])
